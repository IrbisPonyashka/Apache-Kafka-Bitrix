...
class AgentMessageHandler implements MessageHandlerInterface
{
    ...
    public function handle(Message $message): void
    {
        ...
        if (isset($messagePayload["data"])) {
            ...
            $result = $this->bxEntityManager->agent_init($data);
            // $result["data"] = $data;
            
            if ($result["status"] == "success") {
                $this->consumer->commit($message);
                self::Logger($result, "_message_result_", "_message_result");
            } else {
    ...
    {
        $logDir = '/home/bitrix/dev/bx-kafka/logs/agent/';
