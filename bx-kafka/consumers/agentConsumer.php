...
use BxKafka\MessageHandler\AgentMessageHandler;
use Monolog\Logger;
...
try {
    ...
    require '/home/bitrix/dev/bx-kafka/src/MessageHandler/AgentMessageHandler.php';

    $logger = new Logger('agent_consumer');
    $logger->pushHandler(new StreamHandler('/home/bitrix/dev/bx-kafka/logs/agent/agent_consumer.log', Logger::DEBUG));

    $conf = new Conf();

    $conf->set('group.id',                  $kafkaConfig['group_id_agent'] );
    ...
    $consumer->subscribe([$kafkaConfig['topic_agent']]);


    $bxEntityManager = new BxEntityManager();
    $messageHandler = new AgentMessageHandler($consumer, $bxEntityManager);
    $kafkaConsumerHandler = new ConsumerHandler($consumer, $messageHandler, $logger);
    
    $kafkaConsumerHandler->handleMessages();

} catch (Throwable $e) {
    error_log("Fatal error in agentConsumer: " . $e->getMessage());
    exit(1);
}