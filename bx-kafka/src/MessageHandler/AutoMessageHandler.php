...
class AutoMessageHandler implements MessageHandlerInterface
{
    ...
    public function handle(Message $message): void
    {
        ...
        if (isset($messagePayload["data"])) {
            ...
            $result = $this->bxEntityManager->avto_init($data);
            $result["data"] = $data;
            
            if ($result["status"] == "success") {
                $this->consumer->commit($message);
                self::Logger($result, "_message_result__success", "_message_result");
            } else {
                self::Logger($result, "_message_result_error", "_message_result");
            }
        } else {
            self::Logger("Invalid message payload", "_another_message_", "_message");
    ...
    {
        $logDir = '/home/bitrix/dev/bx-kafka/logs/auto/';
